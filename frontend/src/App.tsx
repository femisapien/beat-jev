...
export default function App() {
  ...
  async function dispatch(command: Play) {
    ...
    setError("");
    setTrace(null);
    remember({ matchId: command.matchId, command });
    try {
      const result = await api<{ runId: string }>("/play", command);
      if (seq !== requestSequence.current) return;
      lastRun.current = result.runId;
      remember({ matchId: command.matchId, command, runId: result.runId });
    } catch (e) {
  ...
  const ready = !!game?.ready && view === "aim" && !sending && !error;
    ...
    <div className="app">
          ...
          >
            ...
            {session && view === "flight" && !shot && (
              <div className="pending-pill">
                ...
                Recording your penalty
              </div>
            )}
            {ready && (
              <div className="aim-hint">
                Tap a spot to shoot <span>·</span> Arrows + Space on keyboard
              </div>
            )}
          </div>
              ...
              {Array.from({ length: 5 }, (_, i) => {
                ...
                return (
                  <span
                    ...
                    className={
                      s?.outcome ||
                      (i === visibleShots.length && session ? "current" : "")
                    }
      ...
      {about && (
        <Panel title="How it works" close={() => setAbout(false)}>
          <h2>You shoot. Jev guesses.</h2>
          <p>
            Jev chooses a position from your previous shots. Your next target
            stays hidden until you shoot.
          </p>
          <ol>
            <li>
              <strong>Render Workflows</strong> runs the match, prepares each
              penalty, records the shot, and finalizes the score.
            </li>
            <li>
              <strong>TypeSafe Jev</strong> returns one of six keeper moves with
              its probabilities.
            </li>
            <li>
          ...
          <p>
            Game rules handle the ball, saves, and misses. Wide shots do not
            trigger a dive. The browser animates the saved result.
          </p>
          <p className="muted">
            A nickname is enough. Your record belongs to this browser. Previous
            shot coordinates are sent to TypeSafe; your nickname is not. Open
            the workflow strip to inspect the real task runs and Jev’s latest
            decision.
          </p>
        </Panel>
      )}
