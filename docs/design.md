...
On release, show a 900 ms angled approach, planted support foot, strike, and follow-through. Rotate the body toward the chosen shot direction. The camera follows the approach. Jev shifts weight while waiting, visibly adopts its model-selected starting position, then pushes off, dives, lands, and recovers. Animation implements actions; it does not invent model decisions.
