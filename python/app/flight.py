...
def observe_ball(aim, kick=None):
    ...
    for ms in [0, CONFIG["observationMs"] / 2, CONFIG["observationMs"]]:
        ...
        samples.append(dict(
            ms=ms, x=round(p["x"], 2), y=round(p["y"], 2),
...
def keeper_observation(samples):
    ...
    return dict(
        ...
        measuredMotion=dict(
            atMs=b["ms"], horizontalMetresPerSecond=round(vx, 2),
            verticalMetresPerSecond=round(vy, 2), forwardMetresPerSecond=round(vz, 2),
        ),
