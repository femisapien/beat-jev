...
client = httpx.AsyncClient(timeout=10, headers=headers)
...
async def read_trace(run_id, owner):
    ...
    if not os.getenv("RENDER_LOCAL_DEV_URL"):
        if not WORKFLOW_ID:
            res = await client.get(
                f"{base}/v1/tasks",
                params={"taskSlug": f"{WORKFLOW}/run_game", "limit": 1},
            )
    ...
    response = await client.get(
        f"{base}/v1/task-runs", params={"rootTaskRunId": run_id, "limit": 100}
    )
    response.raise_for_status()
    ids = [
        r["taskRun"]["id"]
    ...
    async def child(run_id):
        run = (
            COMPLETED.get(run_id)
            or (await render.workflows.get_task_run(run_id)).to_dict()
        )
    ...
    children = await asyncio.gather(*(child(i) for i in ids))
    spans = []
    for run in [root, *children]:
        spans.append(
            dict(
                ...
                parentId=run.get("parentTaskRunId"),
                number=run["input"][0].get("number"),
            )
        )
    return dict(
        id=run_id,
